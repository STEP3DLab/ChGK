...
1. **Ведущий** открывает `index.html`, создает игру, переходит в `host.html`, добавляет команды.
2. **Капитаны** подключаются через `join.html`, вводят код капитана, добавляют вопросы.
3. **Ведущий** запускает волчок и раунды в `game.html`, фиксирует ответы и очки.
