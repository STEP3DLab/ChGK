...
1. **Ведущий** открывает `index.html`, создает игру, переходит в `host.html`, добавляет команды и вопросы.
2. **Ведущий** запускает волчок и раунды в `game.html`, вводит ответы и начисляет очки с одного экрана.
3. **Команды и зрители** открывают общий экран через `join.html` → `player.html`.
